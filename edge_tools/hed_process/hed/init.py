...
_dt = Path(__file__).resolve().parents[2]
if str(_dt) not in sys.path:
    sys.path.insert(0, str(_dt))
from paths import HED_CKPT

from .util import HWC3, nms, resize_image_with_pad, safe_step, common_input_validate, custom_hf_download, HF_MODEL_NAME
...
class HEDdetector:
    ...
    def from_pretrained(cls, pretrained_model_or_path=HF_MODEL_NAME, filename="ControlNetHED.pth"):
        # Optional: model_path = custom_hf_download(pretrained_model_or_path, filename)

        netNetwork = ControlNetHED_Apache2()
        netNetwork.load_state_dict(torch.load(HED_CKPT, map_location='cpu'))
    ...
    def __call__(self, input_image, detect_resolution=512, safe=False, output_type="pil", scribble=False, upscale_method="INTER_CUBIC", **kwargs):
        ...
        # input_image, remove_pad = resize_image_with_pad(...)  # disabled: keep full resolution
