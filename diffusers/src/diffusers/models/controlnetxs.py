...
class ControlNetXSModel(ModelMixin, ConfigMixin):
    ...
    def Fourier_filter(self, x, scale=0.1, fft_d=16):
        ...
        mask = torch.full((B, C, H, W), scale, device=x.device, dtype=torch.float32)
        crow, ccol = H // 2, W // 2
        thresh = int(math.ceil(H * (1 / fft_d))) // 2 
        thresw = int(math.ceil(W * (1 / fft_d))) // 2
        thresh = max(1, thresh)
        thresw = max(1, thresw)
        mask[..., crow - thresh : crow + thresh, ccol - thresw : ccol + thresw] = 1.0
        x_freq = x_freq * mask
    ...
    def forward(
        ...
        base_encoder_hidden_states: Optional[torch.Tensor] = None, # 这个将给 UNet 用
        ...
        def process_control_residual(residual, bbox_mask):
            if bbox_mask is None:
                return residual
            
            target_h, target_w = residual.shape[-2:]
            if bbox_mask.shape[-2] != target_h or bbox_mask.shape[-1] != target_w:
                current_bbox_mask = F.interpolate(
                    bbox_mask, 
                ...
                current_bbox_mask = bbox_mask
            
            #only infer, use fft
            if self.training:
                return residual * current_bbox_mask
            
            # 2. 频域分离
            #for high
            res_low = self.Fourier_filter(residual, scale=0.0, fft_d=16)
            
            # 3. 提取高频分量
            res_high = residual - res_low
            low_freq_scale = 0.0
            high_freq_scale = 1.0 
        
            # 组合信号
            processed_residual = (res_high * high_freq_scale) + (res_low * low_freq_scale)
            processed_residual = F.relu(torch.abs(processed_residual) - 0.05) * torch.sign(processed_residual)
            
            
            final_residual = processed_residual  * current_bbox_mask
        ...
        cemb_for_control = encoder_hidden_states
        cemb_for_base = base_encoder_hidden_states if base_encoder_hidden_states is not None else encoder_hidden_states
        ...
        if guided_hint is not None:
            h_ctrl += guided_hint
        ###
        residual = next(it_down_convs_out)(h_ctrl) * next(scales)
        if gating_mask is not None:
            # mask = F.interpolate(gating_mask, size=residual.shape[-2:], mode='bilinear', align_corners=False)
            # residual = residual * mask
            residual = process_control_residual(residual, gating_mask)
        h_base = h_base + residual
        ###
        ...
        for m_base, m_ctrl in zip(base_down_subblocks, ctrl_down_subblocks):
            ...
            h_base = m_base(h_base, temb, cemb_for_base, attention_mask, cross_attention_kwargs) # <-- cemb_for_base
            h_ctrl = m_ctrl(h_ctrl, temb, cemb_for_control, attention_mask, cross_attention_kwargs) # <-- cemb_for_control
            ###
            residual = next(it_down_convs_out)(h_ctrl) * next(scales)
            if gating_mask is not None:
                # mask = F.interpolate(gating_mask, size=residual.shape[-2:], mode='bilinear', align_corners=False)
        ...
        #     h_ctrl = m_ctrl(h_ctrl, temb, cemb, attention_mask, cross_attention_kwargs)  # C - apply ctrl subblock
        for m_base, m_ctrl in zip(base_mid_subblocks, ctrl_mid_subblocks):
            h_base = m_base(h_base, temb, cemb_for_base, attention_mask, cross_attention_kwargs) # <-- 使用 cemb_for_base
            h_ctrl = m_ctrl(h_ctrl, temb, cemb_for_control, attention_mask, cross_attention_kwargs) # <-- 使用 cemb_for_control
        ###
        residual = self.middle_block_out(h_ctrl) * next(scales)
        if gating_mask is not None:
            # mask = F.interpolate(gating_mask, size=residual.shape[-2:], mode='bilinear', align_corners=False)
            # # from IPython import embed;embed()
            # residual = residual * mask
        ...
        for i, m_base in enumerate(base_up_subblocks):
            ###
            residual = next(it_up_convs_out)(hs_ctrl.pop()) * next(scales)
            if gating_mask is not None:
                # mask = F.interpolate(gating_mask, size=residual.shape[-2:], mode='bilinear', align_corners=False)
                # residual = residual * mask
                residual = process_control_residual(residual, gating_mask)
