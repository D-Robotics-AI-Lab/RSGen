...
from . import injection_utils
from . import utils


class Edge2Edge(injection_utils.AttentionBase):
    EPSILON = 1e-5
    FILTER_TAGS = {
    ...
    def __init__(
        ...
        super().__init__()
        self.hbboxes = hbboxes
        self.obboxes = obboxes
        self.boxes = obboxes if obboxes is not None else hbboxes
        ...
        num_clusters = len(self.boxes) * num_clusters_per_box if num_clusters is None else num_clusters
    ...
    def _precompute_obbox_masks(self, default_resolution=64 , device=None):
        #s n
        num_subjects = len(self.obboxes)
        masks = torch.zeros(num_subjects, default_resolution * default_resolution, dtype=bool)
        for i, obbox in enumerate(self.obboxes):
            if np.count_nonzero(obbox) == 0:
                continue
            mask = self._rasterize_obbox(obbox, default_resolution)
            masks[i] = mask.flatten()
        return masks

    @staticmethod
    def _rasterize_obbox(obbox, resolution):
        ...
        points = (points * resolution).astype(np.int32)
        ...
        with torch.enable_grad():
            ...
            for guidance_iter in range(self.max_guidance_iter_per_step):
                if normalized_loss < self.loss_stopping_value:
                    print(normalized_loss, '<', self.loss_stopping_value)
                    break
    ...
    def _hide_other_subjects_from_tokens(self, batch_size, n, d, dtype, device):  # b h i j
        ...
        all_indices_to_mask = []
        for indices in (*self.subject_token_indices, self.filter_token_indices):
            # 77 max
            if indices is None: continue  ###
            valid_indices = [idx for idx in indices if idx < d] ###
            if valid_indices: ###
                sim_masks[:, :, valid_indices] = min_value ###
        ...
        for batch_index in range(batch_size):
            for subject_mask, token_indices in zip(subject_masks[batch_index], self.subject_token_indices):
                ...
                valid_token_indices = [idx for idx in token_indices if idx < d]
                for token_index in valid_token_indices:
                    sim_masks[batch_index, subject_mask, token_index] = 0

        if self.mask_eos and not include_background:
            for batch_index, background_mask in zip(range(batch_size), background_masks):
                if self.eos_token_index < d:
    ...
    def _save_mask_maps(self, attn, is_cross):
        ...
        if is_cross:
            ...
            valid_leading_indices = [idx for idx in self.leading_token_indices if idx < max_len]
            if not valid_leading_indices:
                return 
            attn = attn[..., valid_leading_indices]
    ...
    def _save_loss_values(self, attn, is_cross):
        ...
        if is_cross:
            ...
            for indices in self.subject_token_indices:
                ...
                if self.eos_token_index < max_len:
                    valid_indices.append(self.eos_token_index)
                contexts.append(valid_indices)
            # contexts = [indices + [self.eos_token_index] for indices in self.subject_token_indices]  
        else:
        ...
        for i, (box, context) in enumerate(zip(boxes, contexts)):
            if is_cross and len(context) == 0:
                fg_val = torch.tensor(0.0, device=attn.device)
                bg_val = torch.tensor(0.0, device=attn.device)
            else:
                context_attn = attn[:, :, :, context]
                box_area = box.sum().item()
                bg_area = background_mask.sum().item()
                
                fg_val = context_attn[:, :, box].sum(dim=(1, 2, 3)) / (box_area + 1e-6)
                bg_val = (context_attn[:, :, background_mask].sum(dim=(1, 2, 3)) / (bg_area + 1e-6))  
    ...
    def _convert_boxes_to_masks(self, resolution, device=None):  # 返回 shape: (s, n)
        num_subjects = len(self.boxes)
        masks_2d_list = []  
        
        if self.obboxes is not None:
            for i, obbox in enumerate(self.obboxes):
                mask_2d = self._rasterize_obbox(obbox, resolution)
                masks_2d_list.append(torch.from_numpy(mask_2d).to(device=device).bool())
        else:
            for i, box in enumerate(self.hbboxes):
                mask_2d = torch.zeros(resolution, resolution, dtype=bool, device=device)
                x0, x1 = int(box[0] * resolution), int(box[2] * resolution)
                y0, y1 = int(box[1] * resolution), int(box[3] * resolution)
                
                if x1 > x0 and y1 > y0:
                    mask_2d[y0:y1, x0:x1] = True
                    
                masks_2d_list.append(mask_2d)
        
        # (s, h, w) -> (s, n)
        masks_flat = torch.stack(masks_2d_list).view(num_subjects, -1)
        return masks_flat
