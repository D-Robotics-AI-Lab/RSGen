...
def load_migc(unet, attention_store, pretrained_MIGC_path: Union[str, Dict[str, torch.Tensor]], attn_processor,
    ...
    for key, value in state_dict.items():
        #diffusers
        if key.startswith(("down_blocks", "up_blocks", "mid_block")):   
            # 
            split_idx = key.find(".migc.")
            attn_processor_key = key[:split_idx] # ...attn2.processor
            sub_key = key[split_idx+1:] # migc.norm.weight
    ...
    for key, value_dict in adapter_grouped_dict.items():
        ...
        config = {'not_use_migc': False, 'C': dim}
