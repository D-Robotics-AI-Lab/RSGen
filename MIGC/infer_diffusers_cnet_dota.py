    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                self.unet.eval()
                mask_512 = self.rasterize_union_mask_from_obboxes(obboxes, H=height, W=width, device=latents.device)
...
def parse_args():
    ...
    parser.add_argument("--migc_path", type=str, default="/data/checkpoint-2700/migc.pth")
...
def main():
    ...
    controlnet_weights_path = 'controlnet/diffusion_pytorch_model.safetensors'
    if rank == 0:
    ...
    for line in tqdm(my_lines, disable=(rank != 0), desc=f"Rank {rank}"):
        ...
        image_anno = pipe.draw_box_desc(image_anno, valid_obboxes, valid_phrases)
