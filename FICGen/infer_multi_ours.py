...
class StableDiffusionMIPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        if hed_image is not None:
            # Generate HED map from the provided image
            edge_maps = hed_image
            ...
            for bbox in bboxes[0]:  # Assuming batch_size=1
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if controlnet is not None and controlnet_conditioning_image is not None:
                    ...
                    mask_512 = self.rasterize_union_mask_from_bboxes(bboxes, H=height, W=width, device=latents.device)  # (B,1,512,512)
...
def parse_args():
    ...
    parser.add_argument("--bg_path", type=str, default="/data/train")
    parser.add_argument("--edge_path", type=str, default="/data/egde")
    parser.add_argument("--out_dir", type=str, default="/data/test_new")
...
if __name__ == '__main__':
    ...
    save_path = '/data/checkpoint-11400'
    data_path = '/data/val'
    ...
    controlnet_weights_path = os.path.join('/data/diffusion_pytorch_model.safetensors')
    ...
    for i in range(sample_num):
        ...
        image_anno = pipe.draw_box_desc(image_anno, bboxes[0], prompt[0][1:])
