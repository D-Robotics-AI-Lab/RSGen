...
def main():
    ...
    #todo
    state_dict = load_file(os.path.join('/data/checkpoint-3900', "unet/diffusion_pytorch_model.safetensors"))


    # attn_state_dict = {k: v for k, v in state_dict.items() if ".processor" in k}
    # custom_layers.load_state_dict(attn_state_dict)
    unet.load_state_dict(state_dict, strict=False) 
    ### based on ControlNetXSModel
    ...
    #todo
    image_proj_path = os.path.join('/data/checkpoint-3900', 'ImageProjModel.pth')
    ...
    def preprocess_train(examples):
        ...
        for i, edge_tensor in enumerate(full_res_edge_tensors):
            ...
            # 2. Create Mask (Modified to use Horizontal BBoxes)
            ...
            for bbox in current_bboxes:
                ...
                x_min = int(bbox_np[0] * w)
                y_min = int(bbox_np[1] * h)
                x_max = int(bbox_np[2] * w)
                y_max = int(bbox_np[3] * h)
            ...
            # (H, W, C) -> (C, H, W) logic is preserved
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                mask_512 = rasterize_union_mask_from_bboxes(bboxes, H=height, W=width, device=latents.device)
