...
if __name__ == '__main__':
    cache_name = '/data/hrsc_emb.pt'
    if not os.path.exists(cache_name) or True:
        myCLIP = myCLIPEnc()

        data = []
        with open(os.path.join('/data/HRSC2016/Train/AllImages/metadata.jsonl'), 'r') as f:
            for line in f:
        ...
        sample_data = random.sample(data, 626)
        ...
        for sample in tqdm(sample_data):
            ...
            img = Image.open(os.path.join('/data/AllImages', img_name)).convert('RGB')
