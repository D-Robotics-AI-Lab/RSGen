...
import diffusers
from diffusers import AutoencoderKL, DDPMScheduler, StableDiffusionPipeline, UNet2DConditionModel, ControlNetXSModel 
...
hed_device = None
def get_hed_model():
    global hed_model, hed_device
    if hed_model is None:
        hed_device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        hed_model = HEDdetector.from_pretrained().to(hed_device)
    return hed_model, hed_device

def hed_pred(image, resolution=512, **kwargs):
        model, device = get_hed_model()
        image_tensor = torch.from_numpy(np.array(image).astype(np.float32) / 255.0) 
        image_tensor = image_tensor.unsqueeze(0)
        image_tensor = image_tensor.to(device)
        safe_mode = kwargs.get("safe") == "enable"
        out = common_annotator_call(model, image_tensor, safe=safe_mode)
        return out
...
def main():
    ...
    state_dict = load_file(os.path.join('/data/checkpoint-200', "unet/diffusion_pytorch_model.safetensors"))
    # attn_state_dict = {k: v for k, v in state_dict.items() if ".processor" in k}
    # custom_layers.load_state_dict(attn_state_dict)
    unet.load_state_dict(state_dict, strict=False) 
    ### base on controlnetxs
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(models, weights, output_dir):
            if accelerator.is_main_process:
                models[0].save_pretrained(os.path.join(output_dir, "controlnet"))  
                weights.pop()  
        ...
        def load_model_hook(models, input_dir):
            if os.path.exists(os.path.join(input_dir, 'controlnet/diffusion_pytorch_model.safetensors')):
                logger.info("Loading ControlNet from safetensors...")
                controlnet_state_dict = load_file(os.path.join(input_dir, 'controlnet/diffusion_pytorch_model.safetensors'))
                models[0].load_state_dict(controlnet_state_dict) 
            else:
                logger.warning(f"ControlNet checkpoint not found in {input_dir}")
                
            if args.use_ema:
                if os.path.exists(os.path.join(input_dir, "unet_ema")):
                    # print("Loading EMA UNet...")
                    load_model = EMAModel.from_pretrained(os.path.join(input_dir, "unet_ema"), UNet2DConditionModel)
                    ema_unet.load_state_dict(load_model.state_dict())
                    ema_unet.to(accelerator.device)
                    del load_model
                else:
                    logger.warning(f"EMA UNet checkpoint not found in {input_dir}")
    ...
    def preprocess_train(examples):
        ...
        obboxes_list = [bbox for bbox in examples[obbox_column]]
        # Construct reference cropped instance base
        examples["instances"] = []
        masked_edge_maps = []
        full_res_edge_tensors = [hed_pred(image) for image in images]
        for i, edge_tensor in enumerate(full_res_edge_tensors):
            edge_np = edge_tensor.squeeze().cpu().numpy() # (H, W, C)
            h, w ,c= edge_np.shape
            
            mask = np.zeros((h, w), dtype=np.uint8)
            current_obboxes = obboxes_list[i]
            
            for obbox in current_obboxes:
                if np.count_nonzero(obbox) == 0: 
                    continue
                points = np.array(obbox).reshape(-1, 2)
                points[:, 0] *= w
                points[:, 1] *= h
                cv2.fillPoly(mask, [points.astype(np.int32)], 1)
            
            masked_edge_np = edge_np * mask[..., np.newaxis].astype(edge_np.dtype)
            masked_edge_tensor = torch.from_numpy(masked_edge_np).unsqueeze(0)
            masked_edge_tensor = masked_edge_tensor.permute(0, 3, 1, 2)
            # from IPython import embed;embed()
            #H W C
            masked_edge_maps.append(masked_edge_tensor.squeeze(0))
        examples["edge_maps"] = masked_edge_maps
        examples["instances"] = []
        for index, (caption, bboxes) in enumerate(zip(examples["captions"], examples["bboxes"])):
            ...
            if len(categories) == 0:
                ...
                for name, bbox in zip(categories, bboxes):
                    if name == '' or name not in dict_of_images:
                        instances.append(torch.zeros([3, args.ref_resolution, args.ref_resolution]))
                    else:
                        if len(dict_of_images[name]) > 0:
                            # Safe foreground aspect ratio calculation
                            value = (bbox[2] - bbox[0]) / (bbox[3] - bbox[1] + 1e-10)    
                            nearest_idx = find_nearest(list(dict_of_images[name].values()), value)
                            chosen_file = list(dict_of_images[name].keys())[nearest_idx]
    ...
    def rasterize_union_mask_from_obboxes(obboxes, H, W, device, dilate_px=4, blur_sigma=0.0):
        B = len(obboxes)
    ...
    def collate_fn(examples):
        ...
        edge_maps = torch.stack([example["edge_maps"] for example in examples])
        if edge_maps.shape[-1] != args.resolution or edge_maps.shape[-2] != args.resolution:
            edge_maps = F.interpolate(edge_maps, size=(args.resolution, args.resolution), mode='bilinear')
        return {"pixel_values": pixel_values, "prompts": captions, "instances": instances,"bboxes": bboxes, "obboxes": obboxes, "edge_maps": edge_maps}
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                controlnet_conditioning_image = batch["edge_maps"].to(weight_dtype)
                mask_512 = rasterize_union_mask_from_obboxes(obboxes, H=height, W=width, device=latents.device)
                ...
                model_pred = controlnet(
                    ...
                    noisy_latents,
                    timesteps,
                    encoder_hidden_states=cond_prompt_embeds[0:1],  #bs 1   , deal global prompt
                    base_encoder_hidden_states=cond_prompt_embeds,   
                    gating_mask=mask_512.to(dtype=weight_dtype),
                    controlnet_cond=controlnet_conditioning_image,
                    cross_attention_kwargs=cross_attention_kwargs, 
                    return_dict=False,
                )
                # from IPython import embed;embed()
                # model_pred = model_pred_tuple[0]
